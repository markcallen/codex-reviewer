...
func TestRunDockerLoadsRepoConfigDefaults(t *testing.T) {
	dir := initLocalGitRepo(t)
	writeReviewerConfig(t, dir, `
[review]
base = "origin/release"
profile = "pr-readiness"
policy_file = "POLICY.md"
ignore = ["vendor/**", "dist/**"]
directives = ["Focus on security."]
`)
	t.Setenv("CODEX_API_KEY", "")
	t.Setenv("GITHUB_TOKEN", "")
	...
	for _, want := range []string{
		...
		if !strings.Contains(out, want) {
			t.Fatalf("dry-run output missing config default %q:\n%s", want, out)
		}
	}
}

func TestRunDockerFlagOverridesRepoConfig(t *testing.T) {
	dir := initLocalGitRepo(t)
	writeReviewerConfig(t, dir, `
[review]
base = "origin/release"
...
`)
	...
	if !strings.Contains(out, "origin/main") {
		t.Fatalf("explicit --base flag should override config base:\n%s", out)
	}
	if strings.Contains(out, "origin/release") {
		t.Fatalf("config base should not appear when flag overrides:\n%s", out)
	}
}
