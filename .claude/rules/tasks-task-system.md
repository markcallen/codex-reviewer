...
Use GitHub Issues as the system of record for work items. Check and configure the task system MCP server when asked.

---
# Task System Integration Rules

These rules define how to use GitHub Issues as the system of record for work items and how to set up the required MCP server.
...
This repository uses **GitHub Issues** as the system of record for all planned work, follow-up tasks, bugs, and feature requests. All durable work items must be created there, not left only in local notes or branch files.
...
- "check my MCP setup"
- "configure MCP for GitHub Issues"
...
2. Check whether the correct MCP server for **GitHub Issues** is already configured for that platform (see platform-specific paths below).
...
## Using GitHub Issues for Work Items

- Create issues/tickets in **GitHub Issues** for any planned work, bugs, or follow-up items that extend beyond the current branch.
- When starting a new piece of work, check **GitHub Issues** first for an existing issue to link against.
- When closing a PR, ensure any remaining work has a corresponding issue in **GitHub Issues** — do not leave it only in `tasks/TODO.md`.
...
- If the MCP server is unavailable, fall back to using the **GitHub Issues** web UI and link issues manually in PR descriptions.
